...
/**
 * Performance test for the ProgramItem entity.
 */
class ProgramItemGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ProgramItem entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all programItems")
            .get("/services/surveystore/api/program-items")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new programItem")
            .post("/services/surveystore/api/program-items")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "category":"REQUIRED"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_programItem_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created programItem")
                .get("/services/surveystore${new_programItem_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created programItem")
            .delete("/services/surveystore${new_programItem_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
