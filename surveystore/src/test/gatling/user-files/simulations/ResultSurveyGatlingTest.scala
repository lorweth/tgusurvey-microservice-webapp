...
/**
 * Performance test for the ResultSurvey entity.
 */
class ResultSurveyGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ResultSurvey entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all resultSurveys")
            .get("/services/surveystore/api/result-surveys")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new resultSurvey")
            .post("/services/surveystore/api/result-surveys")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "answer":"OPTION1"
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created resultSurvey")
                .get("/services/surveystore${new_resultSurvey_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created resultSurvey")
            .delete("/services/surveystore${new_resultSurvey_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
