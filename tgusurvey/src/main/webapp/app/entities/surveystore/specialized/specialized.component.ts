...
export class SpecializedComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.specializedService
      ...
      .subscribe(
        (res: HttpResponse<ISpecialized[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInSpecializeds(): void {
    this.eventSubscriber = this.eventManager.subscribe('specializedListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ISpecialized[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/specialized'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
