...
/**
 * Performance test for the SurveyHeader entity.
 */
class SurveyHeaderGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the SurveyHeader entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all surveyHeaders")
            .get("/services/surveystore/api/survey-headers")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new surveyHeader")
            .post("/services/surveystore/api/survey-headers")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "stt":"0"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_surveyHeader_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created surveyHeader")
                .get("/services/surveystore${new_surveyHeader_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created surveyHeader")
            .delete("/services/surveystore${new_surveyHeader_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
