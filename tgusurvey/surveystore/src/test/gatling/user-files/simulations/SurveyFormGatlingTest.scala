...
/**
 * Performance test for the SurveyForm entity.
 */
class SurveyFormGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the SurveyForm entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all surveyForms")
            .get("/services/surveystore/api/survey-forms")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new surveyForm")
            .post("/services/surveystore/api/survey-forms")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created surveyForm")
                .get("/services/surveystore${new_surveyForm_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created surveyForm")
            .delete("/services/surveystore${new_surveyForm_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
