...
/**
 * Performance test for the Specialized entity.
 */
class SpecializedGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Specialized entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all specializeds")
            .get("/services/surveystore/api/specializeds")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new specialized")
            .post("/services/surveystore/api/specializeds")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "mscn":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_specialized_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created specialized")
                .get("/services/surveystore${new_specialized_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created specialized")
            .delete("/services/surveystore${new_specialized_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
