...
/**
 * Performance test for the Students entity.
 */
class StudentsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Students entity")
        ...
        .repeat(2) {
            ...
            .get("/services/userinfo/api/students")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new students")
            .post("/services/userinfo/api/students")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "mssv":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_students_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created students")
                .get("/services/userinfo${new_students_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created students")
            .delete("/services/userinfo${new_students_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
