...
export class SurveyManagerComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.surveyFormService
      ...
      .subscribe(
        (res: HttpResponse<ISurveyForm[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInSurveyForms(): void {
    this.eventSubscriber = this.eventManager.subscribe('surveyFormListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ISurveyForm[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
